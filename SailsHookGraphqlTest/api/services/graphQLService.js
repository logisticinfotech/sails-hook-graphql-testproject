...
module.exports = {
    ...
    createGraphQLTypeForWaterlineModel: function (model, modelID, Node, GraphQLSchemaManager) {
    ...
    createFindArgsTypeForWaterlineModel: function (model, modelID, Node, GraphQLSchemaManager) {
        var attributes = model._attributes;
    ...
    createGraphQLQueries: function (waterlineModel, graphqlType, modelID, GraphQLSchemaManager) {
        var queries = {};
        // query to get by id
        queries[modelID] = {
            type: graphqlType,
        ...
        queries[modelID + 's'] = {
            ...
            args: graphQLService.getFindArgsForWaterlineModel(modelID, GraphQLSchemaManager),
            resolve: (obj, criteria) => {
                ...
                var criteria = JSON.stringify(criteria).replace(/gte/g, '>=').replace(/gt/g, '>').replace(/lte/g, '<=').replace(/lt/g, '<').replace(/neq/g, '!=');
                ...
                if (populate) {
                    ...
                    return wlm.then(function (results) {
                        return results;
                    });
                } else if (aggregate) {
                    if (aggregate[0] == 'count') {
                        ...
                        return waterlineModel.find(whereClause).sum(aggregate[1]).then(function (results) {
                            return results;
                        });
                    } else if (aggregate[0] == 'average') {
                        return waterlineModel.findOne(whereClause).average(aggregate[1]).then(function (results) {
    ...
    createGraphQLMutations: function (waterlineModel, graphqlType, modelID, GraphQLSchemaManager) {
        var mutations = {};
    ...
    getGraphQLSchemaFrom: function (models) {
        ...
        _.each(models, function eachInstantiatedModel(thisModel, modelID) {
            GraphQLSchemaManager.types[modelID] = graphQLService.createGraphQLTypeForWaterlineModel(thisModel, modelID, Node,
                GraphQLSchemaManager);
            GraphQLSchemaManager.findArgsTypes[modelID] = graphQLService.createFindArgsTypeForWaterlineModel(thisModel, modelID, Node, GraphQLSchemaManager);
            GraphQLSchemaManager.queries[modelID] = graphQLService.createGraphQLQueries(thisModel, GraphQLSchemaManager.types[modelID],
                modelID, GraphQLSchemaManager);
        });


        _.each(models, function eachInstantiatedModel(thisModel, modelID) {
            GraphQLSchemaManager.mutations[modelID] = graphQLService.createGraphQLMutations(thisModel, GraphQLSchemaManager.types[modelID],
                modelID, GraphQLSchemaManager);
        });
